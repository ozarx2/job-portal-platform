...
export default function AdminDashboard() {
  ...
  }, [activeTab, leadsPage, leadFilters]);
  ...
  const fetchAllData = async () => {
    try {
      ...
      const [userRes, jobRes, appRes] = await Promise.all([
        axios.get(`${import.meta.env.VITE_API_BASE_URL || 'https://api.ozarx.in/api'}/admin/users`, { headers }),
        axios.get(`${import.meta.env.VITE_API_BASE_URL || 'https://api.ozarx.in/api'}/jobs`, { headers }),
        axios.get(`${import.meta.env.VITE_API_BASE_URL || 'https://api.ozarx.in/api'}/admin/applications`, { headers }),
      ]);
      ...
      // Extract agents from users for filter dropdown
  ...
  const fetchLeads = async () => {
    ...
    try {
      ...
      const response = await axios.get(`${import.meta.env.VITE_API_BASE_URL || 'https://api.ozarx.in/api'}/crm/leads`, {
        headers,
        params
      });
  ...
  const updateLead = async () => {
    try {
      const headers = { Authorization: `Bearer ${token}` };
      await axios.put(
        `${import.meta.env.VITE_API_BASE_URL || 'https://api.ozarx.in/api'}/crm/leads/${editingLead._id}`,
        editingLead,
        { headers }
      );
      setEditingLead(null);
  ...
  const deleteLead = async (id) => {
    ...
    try {
      const headers = { Authorization: `Bearer ${token}` };
      await axios.delete(`${import.meta.env.VITE_API_BASE_URL || 'https://api.ozarx.in/api'}/crm/leads/${id}`, { headers });
      fetchLeads();
    } catch (err) {
