...
export default function CandidateDashboard() {
  ...
  const fetchApplications = async () => {
    const res = await axios.get('http://35.192.180.25:5000/api/applications/me', {
  ...
  const fetchProfile = async () => {
    const res = await axios.get('http://35.192.180.25:5000/api/auth/me', {
      headers: { Authorization: `Bearer ${token}` },
    });
  ...
  const updateProfile = async (e) => {
    ...
    await axios.put('http://35.192.180.25:5000/api/users/update', formData, {
      headers: {
  ...
  const uploadResume = async () => {
    ...
    await axios.post('http://35.192.180.25:5000/api/users/resume', formData, {
      headers: {
        Authorization: `Bearer ${token}`,
        'Content-Type': 'multipart/form-data',
      },
  ...
  const applyJob = async (jobId) => {
    await axios.post(
      `http://35.192.180.25:5000/api/applications/apply`,
      { jobId },
      { headers: { Authorization: `Bearer ${token}` } }
    );
