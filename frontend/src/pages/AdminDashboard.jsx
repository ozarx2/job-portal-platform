...
export default function AdminDashboard() {
  ...
  const fetchCrmDashboard = async () => {
    ...
    try {
      ...
      const response = await axios.get('http://localhost:5000/api/crm/admin/summary', { headers });
  ...
  const fetchLeads = async () => {
    ...
    try {
      ...
      const response = await axios.get('http://localhost:5000/api/crm/leads', {
        headers,
        params
      });
  ...
  const deleteLead = async (id) => {
    ...
    try {
      const headers = { Authorization: `Bearer ${token}` };
      await axios.delete(`http://localhost:5000/api/crm/leads/${id}`, { headers });
      fetchLeads();
    } catch (err) {
